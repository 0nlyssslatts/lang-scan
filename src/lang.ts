...
class Parser {
    ...
    private isEquationStart(): boolean {
        return this.current().type === "int" && this.next()?.value === ":";
    }

    private isSetStart(token = this.current()): boolean {
        return SET_KEYWORDS.has(token.value);
    }
    ...
    private parseBlock(): number {
        ...
        while (this.current().value === "*" || this.current().value === "/") {
            ...
            if (op === "/" && rhs === 0) {
                ...
                throw new ParseError("Деление на ноль", token.line, token.col);
            }
